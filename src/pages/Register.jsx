  ...
  const handleRegister = async e => {
    ...
    try {
      ...
      if (err.response?.data?.errors) {
        ...
        setError(err.response?.data?.msg || 'Не удалось зарегистрироваться');
      }
    } finally {
      setLoading(false);
    }
    ...
    <Container maxWidth="sm" sx={{ mt: 4 }}>
      <Paper elevation={3} sx={{ p: 4, borderRadius: 2 }}>
        ...
        <form onSubmit={handleRegister}>
          <TextField label="Логин" variant="outlined" fullWidth margin="normal" value={name} onChange={e => setName(e.target.value)} required />
