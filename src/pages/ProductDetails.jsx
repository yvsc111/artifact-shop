...
function ProductDetails({ addToCart }) {
  ...
  const handleRatingChange = async (event, newRating) => {
    ...
    try {
      await axios.put(`http://localhost:8000/api/products/${id}/rating`, { rating: newRating });
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            ...
            <Typography variant="h6" color="textSecondary" gutterBottom>
              Прозводитель: {product.brand}
            </Typography>
            <Typography variant="h6" color="textSecondary" gutterBottom>
              Категория: {capitalizeFirstLetter(product.category)}
            </Typography>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', my: 1 }}>
              ...
              <Chip
                label={product.stock > 0 ? `${product.stock} доступно` : 'Нет в наличии'}
